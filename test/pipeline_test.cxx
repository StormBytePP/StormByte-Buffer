...
#ifdef USE_READ
	#define CONSUME(consumer, count, buff) (consumer).Read(count, buff)
#else
	#define CONSUME(consumer, count, buff) (consumer).Extract(count, buff)
#endif
...
int test_pipeline_empty() {
	...
	ASSERT_TRUE("empty pipeline has data", res.has_value());
...
int test_pipeline_single_stage() {
	...
	DataType data;
	auto res = CONSUME(result, 0, data);
	ASSERT_TRUE("single stage has data", res.has_value());
...
int test_pipeline_two_stages() {
	...
	DataType data;
	auto res = CONSUME(result, 0, data);
	ASSERT_TRUE("two stages has data", res.has_value());
...
int test_pipeline_three_stages() {
	...
	DataType data;
	auto res = CONSUME(result, 0, data);
	ASSERT_TRUE("three stages has data", res.has_value());
...
int test_pipeline_incremental_processing() {
	...
	DataType data;
	auto res = CONSUME(result, 0, data);
	ASSERT_TRUE("incremental has data", res.has_value());
...
int test_pipeline_filter_stage() {
	...
	DataType data;
	auto res = CONSUME(result, 0, data);
	ASSERT_TRUE("filter has data", res.has_value());
...
int test_pipeline_multiple_writes() {
	...
	DataType data;
	auto res = CONSUME(result, 0, data);
	ASSERT_TRUE("multiple writes has data", res.has_value());
...
int test_pipeline_large_data() {
	...
	DataType data;
	auto res = CONSUME(result, 0, data);
	ASSERT_TRUE("large data has result", res.has_value());
...
int test_pipeline_reuse() {
	...
	// First use
	{
		...
		Consumer result1 = pipeline.Process(input1.Consumer(), StormByte::Buffer::ExecutionMode::Async, logging);
		wait_for_pipeline_completion(result1);
		
		DataType data1;
		auto res1 = CONSUME(result1, 0, data1);
		ASSERT_TRUE("reuse first has data", res1.has_value());
	...
	// Second use
	{
		...
		Consumer result2 = pipeline.Process(input2.Consumer(), StormByte::Buffer::ExecutionMode::Async, logging);
		wait_for_pipeline_completion(result2);
		
		DataType data2;
		auto res2 = CONSUME(result2, 0, data2);
		ASSERT_TRUE("reuse second has data", res2.has_value());
...
int test_pipeline_copy_constructor() {
	...
	Consumer result = pipeline2.Process(input.Consumer(), StormByte::Buffer::ExecutionMode::Async, logging);
	
	wait_for_pipeline_completion(result);
	
	DataType data;
	auto res = CONSUME(result, 0, data);
	ASSERT_TRUE("copy constructor has data", res.has_value());
...
int test_pipeline_move_constructor() {
	...
	DataType data;
	auto res = CONSUME(result, 0, data);
	ASSERT_TRUE("move constructor has data", res.has_value());
...
int test_pipeline_addpipe_move() {
	...
	DataType data;
	auto res = CONSUME(result, 0, data);
	ASSERT_TRUE("addpipe move has data", res.has_value());
...
int test_pipeline_word_count() {
	...
	DataType data;
	auto res = CONSUME(result, 0, data);
	ASSERT_TRUE("word count has data", res.has_value());
...
int test_pipeline_reverse_string() {
	...
	DataType data;
	auto res = CONSUME(result, 0, data);
	ASSERT_TRUE("reverse has data", res.has_value());
...
int test_pipeline_streaming_data() {
	...
	DataType data;
	auto res = CONSUME(result, 0, data);
	ASSERT_TRUE("streaming has data", res.has_value());
...
int test_pipeline_byte_arithmetic() {
	...
	DataType data;
	auto res = CONSUME(result, 0, data);
	ASSERT_TRUE("byte arithmetic has data", res.has_value());
...
int test_pipeline_large_concurrent_stress() {
	...
	while (result.AvailableBytes() > 0) {
		...
		DataType chunk;
		auto res = CONSUME(result, 2048, chunk); // 2KB chunks (smaller than writer)
		if (res && !chunk.empty()) {
...
int test_pipeline_sync_execution() {
	...
	Consumer result = pipeline.Process(input.Consumer(), StormByte::Buffer::ExecutionMode::Sync, logging);
