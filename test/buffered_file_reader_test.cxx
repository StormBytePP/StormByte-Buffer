...
int test_open_missing_fails() {
	BufferedFileReader in(File("does-not-exist.bin"));
	ASSERT_FALSE("test_open_missing_fails", static_cast<bool>(in));
	ASSERT_FALSE("test_open_missing_fails", in.IsOpen());
	const auto opened = in.Open();
	ASSERT_EQUAL("test_open_missing_fails", Status::Failed, opened.status);
	ASSERT_EQUAL("test_open_missing_fails", static_cast<std::size_t>(0), opened.count);
	ASSERT_FALSE("test_open_missing_fails", in.IsOpen());
	RETURN_TEST("test_open_missing_fails", 0);
}

int test_rewind_without_open_fails() {
	BufferedFileReader in(File("five.bin"));
	ASSERT_EQUAL("test_rewind_without_open_fails", Status::Failed, in.Rewind().status);
...
int test_read_before_open_leaves_dest() {
	...
	ASSERT_EQUAL("test_read_before_open_leaves_dest", Status::Failed, read.status);
...
int test_open_is_idempotent() {
	BufferedFileReader in(File("five.bin"));
	ASSERT_EQUAL("test_open_is_idempotent", Status::Ok, in.Open().status);
	ASSERT_EQUAL("test_open_is_idempotent", Status::Ok, in.Open().status);
	FIFO dest;
	ASSERT_EQUAL("test_open_is_idempotent", Status::Ok, in.Read(5, dest).status);
	ASSERT_EQUAL("test_open_is_idempotent", std::string("ABCDE"), Text(dest));
	RETURN_TEST("test_open_is_idempotent", 0);
}

int test_close_then_read_fails() {
	BufferedFileReader in(File("five.bin"));
	ASSERT_EQUAL("test_close_then_read_fails", Status::Ok, in.Open().status);
	ASSERT_EQUAL("test_close_then_read_fails", Status::Ok, in.Close().status);
	ASSERT_FALSE("test_close_then_read_fails", in.IsOpen());
	ASSERT_EQUAL("test_close_then_read_fails", Status::Ok, in.Close().status);
	FIFO dest("KEEP");
	const auto read = in.Read(1, dest);
	ASSERT_EQUAL("test_close_then_read_fails", Status::Failed, read.status);
	ASSERT_EQUAL("test_close_then_read_fails", std::string("KEEP"), Text(dest));
	ASSERT_EQUAL("test_close_then_read_fails", Status::Failed, in.Rewind().status);
	ASSERT_EQUAL("test_close_then_read_fails", Status::Ok, in.Open().status);
	FIFO again;
	ASSERT_EQUAL("test_close_then_read_fails", Status::Ok, in.Read(5, again).status);
	ASSERT_EQUAL("test_close_then_read_fails", std::string("ABCDE"), Text(again));
	RETURN_TEST("test_close_then_read_fails", 0);
}

int test_rewind_rereads() {
	BufferedFileReader in(File("five.bin"));
	ASSERT_EQUAL("test_rewind_rereads", Status::Ok, in.Open().status);
	FIFO first;
	ASSERT_EQUAL("test_rewind_rereads", Status::Ok, in.Read(5, first).status);
	ASSERT_EQUAL("test_rewind_rereads", Status::Ok, in.Rewind().status);
	ASSERT_EQUAL("test_rewind_rereads", static_cast<std::size_t>(0), in.Tell());
	FIFO second;
	ASSERT_EQUAL("test_rewind_rereads", Status::Ok, in.Read(5, second).status);
...
int test_open_five_read_exact() {
	BufferedFileReader in(File("five.bin"));
	ASSERT_EQUAL("test_open_five_read_exact", Status::Ok, in.Open().status);
	...
	ASSERT_EQUAL("test_open_five_read_exact", Status::Ok, read.status);
...
int test_read_overwrites_dest() {
	BufferedFileReader in(File("five.bin"));
	ASSERT_EQUAL("test_read_overwrites_dest", Status::Ok, in.Open().status);
	FIFO dest("OLD");
	const auto read = in.Read(5, dest);
	ASSERT_EQUAL("test_read_overwrites_dest", Status::Ok, read.status);
...
int test_read_past_end_is_short() {
	BufferedFileReader in(File("five.bin"));
	ASSERT_EQUAL("test_read_past_end_is_short", Status::Ok, in.Open().status);
	FIFO dest;
	const auto read = in.Read(100, dest);
	ASSERT_EQUAL("test_read_past_end_is_short", Status::End, read.status);
...
int test_second_read_after_end() {
	...
	ASSERT_EQUAL("test_second_read_after_end", Status::Ok, in.Open().status);
	FIFO dest;
	ASSERT_EQUAL("test_second_read_after_end", Status::Ok, in.Read(1, dest).status);
	ASSERT_EQUAL("test_second_read_after_end", std::string("A"), Text(dest));
	FIFO again("KEEP");
	const auto end = in.Read(1, again);
	ASSERT_EQUAL("test_second_read_after_end", Status::End, end.status);
...
int test_empty_file() {
	...
	ASSERT_EQUAL("test_empty_file", Status::Ok, in.Open().status);
	const auto size = in.Size();
	ASSERT_TRUE("test_empty_file", size.has_value());
	ASSERT_EQUAL("test_empty_file", static_cast<std::size_t>(0), *size);
	FIFO dest("KEEP");
	const auto read = in.Read(1, dest);
	ASSERT_EQUAL("test_empty_file", Status::End, read.status);
...
int test_peek_does_not_consume() {
	BufferedFileReader in(File("five.bin"));
	ASSERT_EQUAL("test_peek_does_not_consume", Status::Ok, in.Open().status);
	FIFO peek;
	const auto peeked = in.Peek(3, peek);
	ASSERT_EQUAL("test_peek_does_not_consume", Status::Ok, peeked.status);
	...
	FIFO read;
	ASSERT_EQUAL("test_peek_does_not_consume", Status::Ok, in.Read(3, read).status);
...
int test_read_zero_serves_window() {
	...
	ASSERT_EQUAL("test_read_zero_serves_window", Status::Ok, in.Open().status);
	FIFO empty;
	const auto first = in.Read(0, empty);
	ASSERT_EQUAL("test_read_zero_serves_window", Status::Ok, first.status);
	ASSERT_EQUAL("test_read_zero_serves_window", static_cast<std::size_t>(0), first.count);
	FIFO chunk;
	ASSERT_EQUAL("test_read_zero_serves_window", Status::Ok, in.Read(2, chunk).status);
...
int test_sequential_splits() {
	BufferedFileReader in(File("five.bin"));
	ASSERT_EQUAL("test_sequential_splits", Status::Ok, in.Open().status);
	FIFO a, b;
	ASSERT_EQUAL("test_sequential_splits", Status::Ok, in.Read(3, a).status);
	ASSERT_EQUAL("test_sequential_splits", Status::Ok, in.Read(2, b).status);
...
int test_lines_are_octets() {
	...
	ASSERT_EQUAL("test_lines_are_octets", Status::Ok, in.Open().status);
	FIFO dest;
	const auto read = in.Read(18, dest);
	ASSERT_EQUAL("test_lines_are_octets", Status::Ok, read.status);
...
int test_no_nl_and_nul() {
	...
	ASSERT_EQUAL("test_no_nl_and_nul", Status::Ok, text.Open().status);
	FIFO t;
	ASSERT_EQUAL("test_no_nl_and_nul", Status::Ok, text.Read(17, t).status);
	ASSERT_EQUAL("test_no_nl_and_nul", std::string("no newline at end"), Text(t));

	BufferedFileReader raw(File("nul.bin"));
	ASSERT_EQUAL("test_no_nl_and_nul", Status::Ok, raw.Open().status);
	FIFO n;
	ASSERT_EQUAL("test_no_nl_and_nul", Status::Ok, raw.Read(5, n).status);
...
int test_pattern_256() {
	...
	ASSERT_EQUAL("test_pattern_256", Status::Ok, in.Open().status);
	FIFO dest;
	const auto read = in.Read(256, dest);
	ASSERT_EQUAL("test_pattern_256", Status::Ok, read.status);
...
int test_seek_absolute_and_relative() {
	...
	ASSERT_EQUAL("test_seek_absolute_and_relative", Status::Ok, in.Open().status);
	FIFO dest;
	ASSERT_EQUAL("test_seek_absolute_and_relative", Status::Ok, in.Seek(4, Position::Absolute).status);
	ASSERT_EQUAL("test_seek_absolute_and_relative", static_cast<std::size_t>(4), in.Tell());
	ASSERT_EQUAL("test_seek_absolute_and_relative", Status::Ok, in.Read(2, dest).status);
	ASSERT_EQUAL("test_seek_absolute_and_relative", std::string("45"), Text(dest));
	ASSERT_EQUAL("test_seek_absolute_and_relative", Status::Ok, in.Seek(-2, Position::Relative).status);
	ASSERT_EQUAL("test_seek_absolute_and_relative", static_cast<std::size_t>(4), in.Tell());
	FIFO again;
	ASSERT_EQUAL("test_seek_absolute_and_relative", Status::Ok, in.Read(3, again).status);
...
int test_seek_negative_absolute_fails() {
	BufferedFileReader in(File("seek.bin"));
	ASSERT_EQUAL("test_seek_negative_absolute_fails", Status::Ok, in.Open().status);
	ASSERT_EQUAL("test_seek_negative_absolute_fails", Status::Failed, in.Seek(-1, Position::Absolute).status);
...
int test_readahead_knobs() {
	...
	ASSERT_EQUAL("test_readahead_knobs", Status::Ok, in.Open().status);
	FIFO first;
	ASSERT_EQUAL("test_readahead_knobs", Status::Ok, in.Read(5, first).status);
	ASSERT_EQUAL("test_readahead_knobs", std::string("01234"), Text(first));
	std::this_thread::sleep_for(std::chrono::milliseconds(50));
	FIFO second;
	ASSERT_EQUAL("test_readahead_knobs", Status::Ok, in.Read(5, second).status);
...
int test_max_memory_zero_still_reads() {
	...
	ASSERT_EQUAL("test_max_memory_zero_still_reads", Status::Ok, in.Open().status);
	FIFO dest;
	const auto read = in.Read(256, dest);
	ASSERT_EQUAL("test_max_memory_zero_still_reads", Status::Ok, read.status);
...
int test_block_4k_chunked() {
	...
	ASSERT_EQUAL("test_block_4k_chunked", Status::Ok, in.Open().status);
	std::size_t total = 0;
	while (!in.EoF()) {
		FIFO dest;
		const auto read = in.Read(1000, dest);
		if (read.status == Status::Failed)
			return 1;
		total += read.count;
...
int test_move_transfers_session() {
	BufferedFileReader in(File("five.bin"));
	ASSERT_EQUAL("test_move_transfers_session", Status::Ok, in.Open().status);
	FIFO first;
	ASSERT_EQUAL("test_move_transfers_session", Status::Ok, in.Read(2, first).status);
	BufferedFileReader moved(std::move(in));
	ASSERT_FALSE("test_move_transfers_session", static_cast<bool>(in));
	FIFO rest;
	ASSERT_EQUAL("test_move_transfers_session", Status::Ok, moved.Read(3, rest).status);
...
int main() {
	...
	// Session / errors
	// -------------------
	result += test_open_missing_fails();
	result += test_rewind_without_open_fails();
	result += test_read_before_open_leaves_dest();
	result += test_open_is_idempotent();
	result += test_close_then_read_fails();
