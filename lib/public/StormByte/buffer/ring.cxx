		...
		for (std::size_t j = i; j < i + cols; ++j) {
			if (j < line_end) {
				const unsigned int val =
					static_cast<unsigned int>(std::to_integer<unsigned char>(data[j]));
				line << std::hex << std::setw(2) << std::setfill('0')
					<< std::uppercase << val << ' ' << std::dec;
			} else {
				line << "   ";
			}
		}
...
bool Ring::ReadInternal(const std::size_t& count,
							DataType& outBuffer,
	...
	// re-evaluate after possible wait
	...
	switch (flag) {
		case Operation::Read:
			outBuffer.insert(outBuffer.end(), start, start + static_cast<std::ptrdiff_t>(real_count));
...
void Ring::Wait(const std::size_t& n, std::unique_lock<std::mutex>& lock) const {
	if (n == 0)
		return;
	m_cv.wait(lock, [&] {
		if (m_closed || m_error)
