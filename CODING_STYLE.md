...
Include `StormByte/…` first (alphabetical among themselves), then a blank line, then the standard library (alphabetical). Do not `using namespace` in a header. `using namespace StormByte::Buffer;` in a `.cxx` after the includes is fine. Nested namespaces in a `.hxx` stop at three levels; deeper names use `A::B::C::Name`.
...
Types, enumerations and functions are PascalCase (`WriteValue`, `BeginPayload`, `ReadAhead`). Macros are `SCREAMING_SNAKE` (`STORMBYTE_BUFFER_PUBLIC`, `WINDOWS`). One statement per line.
...
`STORMBYTE_BUFFER_PUBLIC` comes **first** on a function declaration. clang-cl rejects `__declspec` after a reference return type.

```
STORMBYTE_BUFFER_PUBLIC FIFO& humanreadable_number(FIFO& fifo) noexcept;
```

Do not write `FIFO& STORMBYTE_BUFFER_PUBLIC Foo();`.

A class keeps the attribute on the type: `class STORMBYTE_BUFFER_PUBLIC FIFO`.
...
extern template STORMBYTE_BUFFER_PUBLIC void FIFO::WriteValue<int>(const int& v);

// .cxx — the body lives only here; consumers must not instantiate
template STORMBYTE_BUFFER_INSTANTIATE void FIFO::WriteValue<int>(const int& v);
```

`STORMBYTE_BUFFER_INSTANTIATE` is `dllexport` on Windows shared builds and empty on ELF (so GCC does not warn `-Wattributes`). Never put it on the `extern` line. Never write `STORMBYTE_BUFFER_PUBLIC extern template`.

Do not repeat `STORMBYTE_BUFFER_PUBLIC` on an ordinary `.cxx` definition. Private implementation types use `STORMBYTE_BUFFER_PRIVATE`.
...
Document every public declaration except `= delete`. Namespaces in a header get a `@namespace` block. Large classes use `@name` groups. `@ref` uses the qualified name (`StormByte::String::String`, `StormByte::System::Device`). Align member `///<` comments to the same column when they fit.
